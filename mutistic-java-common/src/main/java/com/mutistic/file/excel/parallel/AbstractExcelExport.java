...
public abstract class AbstractExcelExport {
  ...
  /**
   * 文件类型 > 默认：WorkTypeEnum.XSSF
   */
  private WorkTypeEnum workType = WorkTypeEnum.XSSF;
  /**
   * Row 每页内容数 > 默认：ExportConst.MIN_ROW_PAGE_SIZE
   */
  private int rowPageSize = ExportConst.MIN_ROW_PAGE_SIZE;
  /**
   * Cell 每页内容数 > 默认：ExportConst.MIN_CELL_PAGE_SIZE
   */
  private int cellPageSize = ExportConst.MIN_CELL_PAGE_SIZE;
   ...
   * @throws Exception
   */
  public abstract <T> String export(String filePrefix, List<T>... rowDates)
      throws Exception;
   ...
   * @param sheet 当前页
   */
  public abstract void buildTop(Sheet sheet);
   ...
   * @param keySet 数据列集合
   */
  public abstract void buildHeader(Sheet sheet, Set<ExportColumn> keySet);
   ...
   * @param rowData   数据源
   * @param <T>       数据类型
   * @param columnMap 字段、标题 关系集合
   * @throws Exception
   */
  public abstract <T> void buildContent(Sheet sheet, List<T> rowData,
      Map<ExportColumn, Field> columnMap) throws Exception;
   ...
   * @return
   */
  public Sheet initSheet(String sheetName) {
...
//    contentStyle.setWrapText(true); // 设置自动换行
  ...
  public void setRowPageSize(Integer rowPageSize) {
    if (rowPageSize == null) {
      return;
    }
    // 暂时不考虑 rowPageSize < ExportConst.MIN_ROW_PAGE_SIZE 情况
//    if (rowPageSize < ExportConst.MIN_ROW_PAGE_SIZE) {
//      return;
//    }
  ...
  public void setCellPageSize(Integer cellPageSize) {
    if (cellPageSize == null) {
      return;
    }
    // 暂时不考虑 cellPageSize < ExportConst.MIN_CELL_PAGE_SIZE 情况
//    if (cellPageSize < ExportConst.MIN_CELL_PAGE_SIZE) {
//      return;
//    }
